...
class Host(object):
    ...
    def endpoint_addresses(self):
        ...
        return (
            ",".join(
                [str(x) for x in self.local_ipv4 + self.local_ipv6 if str(x) > ""]
            ),
        )
...
class Sitecfg:
    ...
    def open_keys(self):
        ...
        if self.publickey:
            ...
            if public_key != self._master_site_key.public_key:
                logger.error(
                    f"Public key in Site config does not match {self.privatekey}"
                )
                sys.exit(1)
        ...
        if (
            self.aws_access_key and self.aws_secret_access_key
        ) and self.aws_credentials == "":
            self._master_aws_secrets = EncryptedAWSSecrets(
            ...
            self._master_aws_secrets = EncryptedAWSSecrets.load_encrypted_credentials(
                self.aws_credentials, box
            )
    ...
    def publish_public_payload(self):
        ...
        return munchify(
            {
                "locus": self.locus,
                "publickey": keyexport(self._master_site_key.public_key),
            }
        )
    ...
    def publish(self):
        ...
        if (
            self._master_aws_secrets and self.aws_credentials == ""
        ) and self._master_site_key:
            box = self.get_message_box(self._master_site_key.public_key)
            self.aws_credentials = (
                self._master_aws_secrets.export_encrypted_credentials(box)
            )
        ...
        if self._master_aws_secrets:
            retval.aws_credentials = (
                self._master_aws_secrets.export_encrypted_credentials(
    ...
    def update_aws_credentials(self, access_key: str, secret_key: str):
        ...
        logger.debug(
            f"Update AWS Credentials: {access_key} / [{'*' * len(secret_key)}]"
        )
...
class Site:
    ...
    def save_site_config(self):
        ...
        publish = {
            "global": unmunchify(sitedata),
            "hosts": {
                h.uuid: unmunchify(h) for h in [h.publish() for h in self.hosts if h]
            },
        }

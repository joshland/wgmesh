...
def decrypt(
    secret_key: PrivateKey, public_key: PublicKey, cipher_text: Union[str, bytes]
) -> bytes:
...
def loadkey(
    key_string: str,
    method: Callable[
        [str, Callable[[str], Union[PublicKey, PrivateKey]]],
...
def keyimport(
    key: str, method: Callable[[str], Union[PublicKey, PrivateKey]]
) -> Union[PrivateKey, PublicKey]:
    """uudecode a key"""
...
if __name__ == "__main__":
    ...
    # *check kets
    assert (
        pubkey == public_import == public_import_from_bytes
    )  # validate the library can export/import public keys
    assert (
        testkey == secret_import == secret_import_from_bytes
    )  # validate that the library can export/import public keys

...
def deploy(locus:           Annotated[str, t.Argument(help='short/familiar name, short hand for this mesh')],
    ...
    for host, values in sync_file.hosts.items():
        ...
        if len(values.remote):
            remotes = ','.join( [ f"{str(x)}:{values.remoteport}" for x in values.remote.split(',') ] )
            ...
            if dry_run:
                ...
                check_update_file(wgconf, os.path.join(deploy_path, f'/etc/wireguard/wg{index}.conf'))
                pass
            continue
        ...
        for x in (deploy_file.ns_private,
